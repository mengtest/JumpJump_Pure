...
public class X_Block:Block
{
	...
	public X_Block (Block parent, BlockType type, int bickNum, Vector3 locStartPot, Vector3 locEndPot, int moveDelay, float moveDuration, Vector3[] moveSpan)
	...
	public override void AdjustBrickNum (int num)
	{
		...
		for (int i=0; i<num; i++) {
			Vector3 locStartPot = new Vector3 (i * Brick.WIDTH, i * Brick.HEIGHT, 0);
	...
	public override void SetBrickMoveInParam ()
	{
		if (m_Bricks.Count > 0 && HasMoveInCondition ()) {
			...
			for (int i=0; i<m_Bricks.Count; i++) {
				...
				if ((moveDirection & Object3d.DIRICTION_UP) > 0 || (moveDirection & Object3d.DIRICTION_RIGHT) > 0) {

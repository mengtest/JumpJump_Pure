...
public class LvMgr
{
	...
	public const int BLOCK_DIFF_NUM = 5;//1;// 5;
	public const int BLOCK_TYPE_NUM = 3;//10;
	public int [] BLOCK_TYPE_NUMS={5,1,1,1,1};
...
//		}
		...
		for (int i=0; i<BLOCK_DIFF_NUM; i++) {
			block_Prefabs [i] = new GameObject[BLOCK_TYPE_NUMS[i]];
			for (int j=0; j<BLOCK_TYPE_NUMS[i]; j++) {
				Debug.Log ("name=" + "Lv_Blocks/Blocks_" + i + "_" + j);
				block_Prefabs [i] [j] = Resources.Load ("Lv_Blocks/Blocks_" + (i + 1) + "_" + j) as GameObject;
			}
		}
	...
	public Block GetRandomBlock ()
	{
		...
		//test
//		diffId = 3;
		diffId=Mathf.Clamp(diffId++,1,BLOCK_DIFF_NUM); // drop the head 
